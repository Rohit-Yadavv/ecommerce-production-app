...
export const registerController = async (req, res) => {
    try {
        ...
        if (!answer) return res.send({ message: 'address is required' });
...
export const forgetPasswordController = async (req, res) => {
    try {
        ...
        if (!newPassword) return res.send({ message: 'newPassword is required' });

        const user = await userModel.findOne({ email, answer });
        if (!user) return res.send({ success: false, message: 'wrong email or answer' });
